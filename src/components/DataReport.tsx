...
const DataReport: React.FC<DataReportProps> = ({ cashFlows, tranches, onDownload }) => {
  ...
  const visibleFlows = isExpanded ? cashFlows : cashFlows.slice(0, 24);
    ...
    <div className="bg-charcoal p-6 rounded-xl border border-white-subtle shadow-lg">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-text text-sm mt-1">Monthly principal and interest distribution across the capital stack.</p>
        </div>
        <button 
          onClick={onDownload}
      ...
      <div className="overflow-x-auto rounded-lg border border-white-subtle/10">
        <table className="w-full text-xs text-right text-slate-text font-mono border-collapse">
          <thead className="bg-deep-navy text-silver-text sticky top-0 z-10">
            <tr>
              <th className="p-4 text-left font-bold uppercase tracking-wider border-b border-white-subtle/20">Period</th>
              <th className="p-4 font-bold uppercase tracking-wider border-b border-white-subtle/20">Pool Bal (End)</th>
              <th className="p-4 font-bold uppercase tracking-wider border-b border-white-subtle/20 text-inst-blue">Interest</th>
              <th className="p-4 font-bold uppercase tracking-wider border-b border-white-subtle/20 text-strat-orange">Principal</th>
              <th className="p-4 font-bold uppercase tracking-wider border-b border-white-subtle/20 text-risk-red">Defaults</th>
              {tranches.map(t => (
                <React.Fragment key={t.id}>
                  <th className="p-4 border-l border-white-subtle/20 font-bold uppercase tracking-wider border-b border-white-subtle/20">{t.name} Int</th>
                  <th className="p-4 font-bold uppercase tracking-wider border-b border-white-subtle/20">{t.name} Prin</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-white-subtle/5">
            ...
            {/* Summary Totals Row */}
            <tr className="bg-deep-navy/50 font-bold text-silver-text border-t-2 border-convexica-gold/30">
      ...
      <div className="mt-4 flex justify-center">
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 text-convexica-gold hover:text-convexica-gold/80 text-sm font-bold transition-colors"
        >
          {isExpanded ? (
            <><ChevronUp className="w-4 h-4" /> Show Initial 24 Months</>
          ) : (
            <><ChevronDown className="w-4 h-4" /> View Full Life of Deal ({cashFlows.length} Months)</>
          )}
        </button>
      </div>
    </div>
